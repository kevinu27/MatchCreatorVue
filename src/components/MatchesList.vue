<template>
  <li v-for="(match, index) in $store.state.matches" v-bind:key="index" class="PlayerNameList" >
    ...
    <div class="matchCard" v-if="$store.state.selectedMatchVS === '1vs1'"> <p>{{match.teams[0].members[0].name}}<span> VS</span>   {{match.teams[1].members[0].name}} </p> <div class="inputScore"><input type="text" v-on:input="setMatchScore_1vs1($event, match.matchId)"> <p>1v1*</p><input type="text" v-on:input="setMatchScore2_1vs1($event,  match.matchId)"> </div></div>
  </li>
